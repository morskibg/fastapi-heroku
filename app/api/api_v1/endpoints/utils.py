...
def excel(
    ...
    try:
        ...
        df = pd.read_excel(
            os.path.join(os.getcwd(), 'stp2021_parts.xlsx'), sheet_name=code.split('_')[0], index_col=0)
        # print(df)

        raw_df = df[(df['code'] == code) & (df.index >= start_date_utc)
        ...
        month_end_date = start_date_obj.replace(day=calendar.monthrange(
            start_date_obj.year, month=int(start_date_obj.month))[1])
        ...
        monthly_df = df[(df['code'] == code) & (df.index >= month_start_date_utc)
        ...
        weekend_df = monthly_df[~monthly_df['is_work']].groupby([
                                                                'hour']).mean()
    ...
    return [{code: raw_df.to_json(orient='index')},
            {f'{calendar.month_name[start_date_obj.month]}-{start_date_obj.year}': work_df.to_json(
                orient='index')},
            {f'{calendar.month_name[start_date_obj.month]}-{start_date_obj.year}-WeekEnd': weekend_df.to_json(orient='index')}]
